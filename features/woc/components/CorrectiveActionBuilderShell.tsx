...
const futureExtractionFields = Object.keys(mockExtractedRouterData).map((key) => {
...
export function CorrectiveActionBuilderShell() {
  ...
  return (
    <article className="card corrective-action-builder-shell">
      ...
      <p className="field-help">Frontend mock/planning only. AI extraction, backend storage, PDF generation, GitHub automation, and runtime agent execution are not enabled in V3-M28.</p>

      <section className="card" aria-label="Router Capture and Auto-Populate Planning" style={{ marginTop: 16 }}>
        <div className="card-header"><div><span className="step-pill">01</span><h3>Router / Work Order Capture</h3><p>Upload or photograph router/work-order evidence for future AI-assisted field extraction.</p></div><span className="field-status pending">Planning</span></div>
        ...
        <div className="form-grid" style={{ marginTop: 16 }}><label>Optional Router Upload Placeholder<input type="file" disabled /></label></div>
        <section className="agent-workflow-grid" aria-label="Future extraction fields" style={{ marginTop: 16 }}>
          <article className="card agent-workflow-card"><div className="card-header"><div><h3>Future Auto-Populate Fields</h3><p>Future AI Vision extraction targets.</p></div></div><div className="placeholder-list">{futureExtractionFields.map((field) => <div className="placeholder-item" key={field}><strong>{field}</strong><span>Future AI-assisted extraction target</span></div>)}</div></article>
          <article className="card agent-workflow-card"><div className="card-header"><div><h3>Human Verification Gate</h3><p>AI can populate fields. Human must confirm before release.</p></div></div><div className="placeholder-list"><div className="placeholder-item"><strong>Verification Rule</strong><span>Extracted data must be reviewed and confirmed by the user before being used in the corrective action preview or future PDF.</span></div></div></article>
        </section>
        <div className="form-grid" style={{ marginTop: 16 }}><button className="button secondary" type="button" disabled>Capture Router Coming Soon</button><button className="button secondary" type="button" disabled>Auto-Populate Coming Soon</button><button className="button secondary" type="button" disabled>Confirm Extracted Data Coming Soon</button></div>
      </section>

      <section className="card" aria-label="Router Data Extraction Preview" style={{ marginTop: 16 }}>
        <div className="card-header"><div><span className="step-pill">02</span><h3>Router Data Extraction Preview</h3><p>Mock AI output preview for future router/work-order extraction review.</p></div><span className="field-status pending">Mock Only</span></div>
        <p className="field-help">AI-extracted data must be reviewed and confirmed before being used in preview/PDF. Applying mock data does not confirm it.</p>
        <div className="action-row" style={{ marginTop: 12 }}><span className="field-status pending">Mock extraction only</span><span className="field-status pending">Needs user verification</span><span className="field-status pending">Not confirmed</span></div>
        <div className="placeholder-list" style={{ marginTop: 16 }}><div className="placeholder-item"><strong>Review Status</strong><span>{extractionReviewStatus}</span></div></div>
        <section className="agent-workflow-grid" aria-label="Mock extracted router data" style={{ marginTop: 16 }}>
          {Object.entries(mockExtractedRouterData).map(([key, value]) => (
            <article className="card agent-workflow-card" key={key}><div className="card-header"><div><h3>{futureExtractionFields[Object.keys(mockExtractedRouterData).indexOf(key)]}</h3><p>{value}</p></div></div></article>
          ))}
        </section>
        <button className="button secondary full-width" type="button" onClick={applyMockExtractedData} style={{ marginTop: 16 }}>Apply Mock Extracted Data</button>
      </section>

...
export function CorrectiveActionBuilderShell() {
  ...
  return (
    <article className="card corrective-action-builder-shell">
      ...
      <p className="field-help">Frontend mock/planning only. AI extraction, backend storage, PDF generation, GitHub automation, and runtime agent execution are not enabled in V3-M29.</p>
      ...
      <section className="card" aria-label="Router Data Extraction Preview" style={{ marginTop: 16 }}>
        ...
        <div className="action-row" style={{ marginTop: 12 }}><span className="field-status pending">Mock extraction only</span><span className="field-status pending">Needs user verification</span><span className={routerDataConfirmed ? 'field-status confirmed' : 'field-status pending'}>{routerDataConfirmed ? 'Confirmed by User' : 'Not Confirmed'}</span></div>
        <div className="placeholder-list" style={{ marginTop: 16 }}><div className="placeholder-item"><strong>Verification Status</strong><span>{routerDataConfirmed ? 'Confirmed by User' : 'Not Confirmed'}</span></div><div className="placeholder-item"><strong>Ready for Preview/PDF</strong><span>{routerDataConfirmed ? 'Yes' : 'No'}</span></div><div className="placeholder-item"><strong>Review Message</strong><span>{verificationMessage}</span></div><div className="placeholder-item"><strong>Review Status</strong><span>{extractionReviewStatus}</span></div></div>
        <section className="agent-workflow-grid" aria-label="Mock extracted router data" style={{ marginTop: 16 }}>{Object.entries(mockExtractedRouterData).map(([key, value]) => <article className="card agent-workflow-card" key={key}><div className="card-header"><div><h3>{routerFieldLabels[key as keyof CorrectiveActionData] || key}</h3><p>{value}</p></div></div></article>)}</section>
        <div className="form-grid" style={{ marginTop: 16 }}><button className="button secondary" type="button" onClick={applyMockExtractedData}>Apply Mock Extracted Data</button><button className="button success" type="button" onClick={confirmExtractedData}>Confirm Extracted Data</button><button className="button danger" type="button" onClick={resetConfirmation}>Reset Confirmation</button></div>
      </section>

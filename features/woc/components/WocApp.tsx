...
import {
  ...
  getEffectiveAffectedOperationEquipment,
  getEffectiveAffectedProcess,
  getGateStatus,
...
export function WocApp() {
  ...
  const applyExtractedData = (extracted: ExtractedWorkOrderData) => {
    setWocData((current) => ({
      ...
      // Extracted router departments/operations are supporting context only; Simple Mode selections remain the report context.
      foundAtDepartment: current.foundAtDepartment,
      affectedArea: current.affectedArea,
      affectedOperationEquipment: current.affectedOperationEquipment,

...
- If the operator reports an incorrect time/rate, runtime, cycle-time, parts-per-hour, or router-standard mismatch, default to mild factual review language: track actual output during the review period, use the observed baseline for scheduling review until Engineering confirms the router update, review current production impact before changing the router standard, and state that no production hold is recommended unless separately directed by supervision or quality.
- For time/rate issues, use cautious verbs such as review, verify, confirm, compare, document, and update router if validated. Do not automatically stop, hold, quarantine, scrap, rework, notify the customer, or escalate safety.

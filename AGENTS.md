...
- Never merge, deploy, rename GitHub or Vercel, or change production settings
  automatically.
- The AI-CAS Foreman implementation workflow is manual `workflow_dispatch` only. It may bootstrap a selected milestone only when no active implementation PR exists. Once a PR exists, `Continue AI-CAS` repairs that same branch and PR through Codex; no parallel repair branch or duplicate PR is allowed.
